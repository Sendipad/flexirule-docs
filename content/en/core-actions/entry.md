...
### Variable Context
Available through the `vars` object. Variables are initially empty and can be populated by [Assignment]({{< relref "core-actions/assignment.md" >}}) actions.
...
- **Next Node**: A [Condition]({{< relref "core-actions/condition" >}}) node checking `doc.customer_group`.
...
- **Immediate Branching**: If your rule handles multiple scenarios, follow the Entry Action immediately with a [Condition]({{< relref "core-actions/condition" >}}) or [Switch]({{< relref "core-actions/switch" >}}) node.
...
- **Expecting Mutation**: Attempting to perform data changes within the Entry Action itself. Use an [Assignment]({{< relref "core-actions/assignment" >}}) node immediately after the Entry Action for initialization.
...
## Related Topics
- [Execution Semantics]({{< relref "advanced-reference/reference/execution/entry.md" >}})
- [Architecture Reference]({{< relref "advanced-reference/architecture/actions/entry.md" >}})
- [Triggers Overview]({{< relref "advanced-reference/triggers/_index.md" >}})
