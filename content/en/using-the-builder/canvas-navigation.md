...
### Working with Blocks
In FlexiRule, every step in your logic is represented by a **Block**.
- **Select**: Click a block to open its configuration panel on the right.
- **Move**: Click and drag a block to reposition it.
...
### The Action Zone
The **Action Zone** is the interactive space around blocks and connection points.
- **Quick Add**: Hover over a connection line or a port (the circles on the sides of a block) to see the `+` icon. Clicking this opens the Action Menu to insert a new block at that exact location.
...
When you run a test, the builder highlights the exact path taken during execution in green, showing you exactly which blocks were triggered.

{{< video src="/images/debug-rule-view-execution-path.webm" >}}

### Inspecting Results
You can click on any block after a test run to see its specific results, including modified variables and data returned by that step.
